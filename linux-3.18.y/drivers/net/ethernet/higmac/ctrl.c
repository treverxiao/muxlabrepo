...
void higmac_hw_get_mac_addr(struct higmac_netdev_local *ld, unsigned char *mac)
{
	...
	reg = readl(ld->gmac_iobase + STATION_ADDR_HIGH);
	mac[0] = (reg >> 8) & 0xff;
	mac[1] = reg & 0xff;

	reg = readl(ld->gmac_iobase + STATION_ADDR_LOW);
	mac[2] = (reg >> 24) & 0xff;
	mac[3] = (reg >> 16) & 0xff;
	mac[4] = (reg >> 8) & 0xff;
	mac[5] = reg & 0xff;
...
int higmac_hw_set_mac_addr(struct higmac_netdev_local *ld, unsigned char *mac)
{
	...
	reg = mac[1] | (mac[0] << 8);
	writel(reg, ld->gmac_iobase + STATION_ADDR_HIGH);

	reg = mac[5] | (mac[4] << 8) | (mac[3] << 16) | (mac[2] << 24);
	writel(reg, ld->gmac_iobase + STATION_ADDR_LOW);
