...
void higmac_set_macif(struct higmac_netdev_local *priv, int mode, int speed)
{
	u32 val;

	/* enable change: port_mode */
	writel(BIT_MODE_CHANGE_EN, priv->gmac_iobase + MODE_CHANGE_EN);
	if (speed == SPEED_1000)
		val = GMAC_SPEED_1000;
		...
		val = GMAC_SPEED_10;
	writel(val, priv->gmac_iobase + PORT_MODE);
	/* disable change: port_mode */
	writel(0, priv->gmac_iobase + MODE_CHANGE_EN);

	/* soft reset mac_if */
	reset_control_assert(priv->macif_rst);

	/* config mac_if */
	writel((u32)mode, priv->macif_base);
