...
/*add for TC_NS_SharedMemoryRelease no using*/
#if 0
...
static int calc_buf_hash(unsigned char *data, unsigned long len, char *digest){
	...
	if(!ca_shash_tfm){
		...
		if (IS_ERR(ca_shash_tfm)) {
			...
			TCERR("Can not allocate %s (reason: %ld)\n", rc);
			return rc;
		}
	}
...
find_service:
	...
	if (context->login.method == TEEC_LOGIN_IDENTIFY) {
	    ...
	    TCERR("login method is not supported\n");
	    ret = -EINVAL;
	    goto error;
	}
...
static int TC_NS_TST_CMD(TC_NS_DEV_File *dev_id, void *argp)
{
	...
	switch (cmd_id) {
	case TST_CMD_01:
		//TODO: XXX, by lijianhui, fix compiling error
#ifdef TC_ASYNC_NOTIFY_SUPPORT
		callback_demo_main((char *)client_context.uuid);
#else
			TCERR("not define TC_ASYNC_NOTIFY_SUPPORT\n");
#endif
		break;
	case TST_CMD_02:
...
static __init int tc_init(void)
{
	...
	// irq = irq_of_parse_and_map(np, 0);   //TODO: not support DTS
#if defined(CONFIG_ARCH_HI3798MX)
	irq = 93; //TODO: current use a reserve IRQ 61 + 32
...
//	mutex_init(&tee_crypto_hash_lock);
    ...
    /* add by yangwei for init smmu ta, and hi3798mx is not supported smmu */
#ifdef CONFIG_HISI_IOMMU_API
