...
enum TEEC_SharedMemCtl {
	TEEC_MEM_INPUT = 0x1,
			   /**< 共享内存的数据流是从客户端应用到安全服务  */
	TEEC_MEM_OUTPUT = 0x2,
			    /**< 共享内存的数据流是从安全服务到客户端应用  */
	TEEC_MEM_INOUT = 0x3,
			   /**< 共享内存可在客户端应用与安全服务之间双向传递  */
};
...
enum TEEC_ParamType {
	...
	TEEC_VALUE_INPUT = 0x01,
			      /**< 与类型#TEEC_Value相对应，只能作为输入，
    数据流是从客户端应用到安全服务*/
	TEEC_VALUE_OUTPUT = 0x02,
			       /**< 与类型#TEEC_Value相对应，只能作为输出，
    数据流是从安全服务到客户端应用*/
	TEEC_VALUE_INOUT = 0x03,
			      /**< 与类型#TEEC_Value相对应，既可输入也可输出  */
	TEEC_MEMREF_TEMP_INPUT = 0x05,
				    /**< 与类型#TEEC_TempMemoryReference相对应，
	...
	TEEC_ION_INPUT = 0x08,	/**< 与类型#TEEC_IonReference相对应，只能作为输入,
				数据流是从客户端应用到安全服务*/
	TEEC_MEMREF_WHOLE = 0xc,
				  /**< 与类型#TEEC_RegisteredMemoryReference
    相对应，引用整块内存 ，数据流与所指向的共享内存的标识#TEEC_SharedMemCtl一致 */
	TEEC_MEMREF_PARTIAL_INPUT = 0xd,
			/**< 与类型#TEEC_RegisteredMemoryReference相对应，
    只能作为输入，数据流是从客户端应用到安全服务 */
