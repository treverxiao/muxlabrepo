...
static struct dentry *__wp_entry = NULL;
...
static int fs_ctrl_update(struct fs_item *item)
{
	...
	if (!item->flags && !item->midmask) {
		/* cancel watchpoint for this mid & page. */
...
static int fs_wp_insert(ulong phy_pageoff, ulong virt_pageoff, int nr_pages,
...
exit:
	mutex_unlock(&__fs_wp->mutex);

	return 0;
}
/******************************************************************************/

static int fs_wp_remove(ulong phy_pageoff, int nr_pages, u32 midmask, u32 flags)
{
	struct fs_item *item;
	...
	while (nr_pages-- > 0 && phy_pageoff < MAX_PHY_PAGEOFF) {
		...
		if (item) {
			...
			/* if item->midmask == 0, need remove this item. */
			if (!item->flags && !item->midmask)
				fs_item_remove(item);
...
static int lock_kerneltext(void)
{
	...
	fs_wp_insert(phyaddr >> PAGE_SHIFT, viraddr >> PAGE_SHIFT, nr_pages, midmask, WP_KERNELTEXT);
...
static int fs_item_init(void)
{
	...
	*item = NULL;
	...
	mutex_init(&__fs_wp->mutex);

	return 0;
}
/******************************************************************************/

static int ddr_wp_init(void)
{
	int ret;
				 ...
				 size_t sz_ubuf, loff_t *ppos)
{
	...
	while (ptr < ptail) {
		...
		nr_pages = (int)((e_physaddr + PAGE_SIZE - 1) >> PAGE_SHIFT) - (s_physaddr >> PAGE_SHIFT) + 1;
		...
		if (enable) {
			fs_wp_insert((ulong)virt_to_phys((void *)s_virtaddr) >> PAGE_SHIFT, s_virtaddr >> PAGE_SHIFT, nr_pages, midmask, WP_KERNELTEXT);
		} else {
			fs_wp_remove((ulong)virt_to_phys((void *)s_virtaddr) >> PAGE_SHIFT, nr_pages, midmask, WP_KERNELTEXT);
		}
	}
...
static int __init ddr_wp_module_init(void)
{
	...
	ret = platform_driver_register(&hiwp_pltm_driver);
	if (ret)
		return ret;

	__wp_entry = debugfs_create_file("ddr_wp", 0644, NULL, NULL,
		&ddr_wp_fops);
	if (!__wp_entry) {
...
module_init(ddr_wp_module_init);
