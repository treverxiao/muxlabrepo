...
ifeq ($(CFG_FPGA_VERSION_UMAP5),y)
CONFIG_HI3798MV2X_FPGA = 1
KERNEL_PATCH_DIR := $(KERNEL_DIR)/$(KERNEL)_umap5.patch
endif

ifeq ($(CFG_FPGA_VERSION_UMAP6),y)
CONFIG_HI3798MV2X_FPGA = 1
endif
...
INITRAMFS_SOURCE = $(HI_ROOTFS_DIR)
endif
...
ifeq ($(DISABLE_TEE),1)
ifeq ($(CONFIG_HI3798MV2X_FPGA),1)
$(FIP_IMG): $(KERNEL_OUT_DIR)/arch/$(CFG_HI_CPU_ARCH)/boot/uImage
	make -C $(FIP_SRC_DIR) CROSS_COMPILE=$(CROSS_COMPILE) PLAT=$(CFG_HI_CHIP_TYPE) SPD=none CONFIG_HI3798MV2X_FPGA=$(CONFIG_HI3798MV2X_FPGA) BL33=$< DEBUG=1 DISABLE_TEE=$(DISABLE_TEE) fip O=$(FIP_OUT_DIR) ATF_EXT_FLAGS=$(ATF_EXTRA_FLAGS)
else
$(FIP_IMG): $(KERNEL_OUT_DIR)/arch/$(CFG_HI_CPU_ARCH)/boot/uImage
...
ifeq ($(CONFIG_HI3798MV2X_FPGA),1)
$(FIP_IMG): $(KERNEL_OUT_DIR)/arch/$(CFG_HI_CPU_ARCH)/boot/uImage
	make -C $(FIP_SRC_DIR) CROSS_COMPILE=$(CROSS_COMPILE) PLAT=$(CFG_HI_CHIP_TYPE) CONFIG_HI3798MV2X_FPGA=$(CONFIG_HI3798MV2X_FPGA) NEED_BL32=no BL33=$< DEBUG=1 fip O=$(FIP_OUT_DIR) ATF_EXT_FLAGS=$(ATF_EXTRA_FLAGS)
else
$(FIP_IMG): $(KERNEL_OUT_DIR)/arch/$(CFG_HI_CPU_ARCH)/boot/uImage
...
fs_buildin: prepare $(INITRAMFS_SOURCE)
	make -C $(KERNEL_SRC_DIR) ARCH=$(CFG_HI_CPU_ARCH) \
		...
		uImage
ifeq ($(CFG_HI_CPU_ARCH),arm64)
	make -C $(FIP_SRC_DIR) CROSS_COMPILE=$(CROSS_COMPILE) PLAT=$(CFG_HI_CHIP_TYPE) $(if $(DISABLE_TEE), SPD=none,) BL33=$(KERNEL_OUT_DIR)/arch/$(CFG_HI_CPU_ARCH)/boot/uImage DEBUG=1 DISABLE_TEE=$(DISABLE_TEE) CONFIG_HI3798MV2X_FPGA=$(CONFIG_HI3798MV2X_FPGA) fip O=$(FIP_OUT_DIR)
endif
ifeq ($(CFG_HI_LOADER_APPLOADER),y)
...
linux_unprepare:
	$(AT)rm -rf $(LINUX_DIR)/drivers/common $(LINUX_DIR)/drivers/msp $(LINUX_DIR)/drivers/wifi
